...
OtherHolidayManagementVo vo = (OtherHolidayManagementVo)params.getVo();
%>
<div class="List">
	<table class="InputTable">
		...
		<td class="InputTd" id="tdSearchOtherHolidayDate">
			...
			<button type="button" class="Name2Button" id="btnActivateDate"onclick="submitForm(event, 'tdSearchOtherHolidayDate', null, '<%= OtherHolidayManagementAction.CMD_SET_ACTIVATION_DATE %>');"><%= vo.getModeActivateDate().equals(PlatformConst.MODE_ACTIVATE_DATE_FIXED) ? params.getName("Change") : params.getName("Decision") %></button>
		</td>
	...
	<table class="ButtonTable">
		<tr>
			<td class="ButtonTd" id="">
				<button type="button" id="btnRegist" class="Name2Button" onclick="submitTransfer(event, null, null, null, '<%= OtherHolidayManagementAction.CMD_SEARCH %>');"><%= params.getName("Search") %></button>
...
<div class="List">
	<table class="LeftListTable" id="list">
		<tr>
			...
			<th class="ListSortTh" id="thActivateDate"		onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_SORT_KEY %>', '<%= ActivateDateComparator.class.getName() %>'), '<%= OtherHolidayManagementAction.CMD_SORT %>');"><%= params.getName("ActivateDate") %><%= PlatformUtility.getSortMark(ActivateDateComparator.class.getName(), params) %></th>
			<th class="ListSortTh" id="thEmployeeName"		onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_SORT_KEY %>', '<%= EmployeeNameComparator.class.getName() %>'), '<%= OtherHolidayManagementAction.CMD_SORT %>');"><%= params.getName("Employee","FirstName") %><%= PlatformUtility.getSortMark(EmployeeNameComparator.class.getName(), params) %></th>
			<th class="ListSortTh" id="thSection"			onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_SORT_KEY %>', '<%= SectionCodeComparator.class.getName() %>'), '<%= OtherHolidayManagementAction.CMD_SORT %>');"><%= params.getName("Section") %><%= PlatformUtility.getSortMark(SectionCodeComparator.class.getName(), params) %></th>
			<th class="ListSortTh" id="thHolidayType"		onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_SORT_KEY %>', '<%= HolidayManagementListHolidayCodeComparator.class.getName() %>'), '<%= OtherHolidayManagementAction.CMD_SORT %>');"><%= params.getName("Vacation","Classification") %><%= PlatformUtility.getSortMark(HolidayManagementListHolidayCodeComparator.class.getName(), params) %></th>
			<th class="ListSortTh" id="thHolidayRemainder"	onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_SORT_KEY %>', '<%= HolidayManagementListHolidayRemainderComparator.class.getName() %>'), '<%= OtherHolidayManagementAction.CMD_SORT %>');"><%= params.getName("Remainder","Days") %><%= PlatformUtility.getSortMark(HolidayManagementListHolidayRemainderComparator.class.getName(), params) %></th>
			<th class="ListSortTh" id="thHolidayLimit"		onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_SORT_KEY %>', '<%= HolidayManagementListHolidayLimitComparator.class.getName() %>'), '<%= OtherHolidayManagementAction.CMD_SORT %>');"><%= params.getName("Acquisition","TimeLimit") %><%= PlatformUtility.getSortMark(HolidayManagementListHolidayLimitComparator.class.getName(), params) %></th>
		</tr>
<%
...
%>
		<tr>
			<td class="ListSelectTd">
				<button type="button" onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_ACTIVATE_DATE %>', '<%= HtmlUtility.escapeHTML(vo.getAryLblActivateDate()[i]) %>', '<%= PlatformConst.PRM_TRANSFERRED_CODE %>' , '<%= HtmlUtility.escapeHTML(vo.getAryLblEmployeeCode()[i]) %>','<%= TimeConst.PRM_TRANSFERRED_GENERIC_CODE %>','<%= HtmlUtility.escapeHTML(vo.getAryLblHolidayCode()[i]) %>'), '<%= OtherHolidayHistoryAction.CMD_SELECT_SHOW %>');"><%= params.getName("History") %></button>
			</td>
...
%>
<div class="Button">
	<button type="button" id="btnOtherHolidayHistory" class="Name4Button" onclick="submitTransfer(event, null, null, null, '<%= OtherHolidayHistoryAction.CMD_SHOW %>');"><%= params.getName("New","Insert") %></button>
</div>
<%
if (vo.getList().isEmpty()) {
	return;
