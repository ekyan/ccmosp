	...
	<table class="LeftListTable LeftRequestListTable" id="list">
		<thead>
			<tr>
				...
				<th class="ListSortTh" id="thApprover" onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_SORT_KEY %>', '<%= ManagementRequestApproverNameComparator.class.getName() %>'), '<%= CancellationRequestAction.CMD_SORT %>')"><%= params.getName("Approver") %><%= PlatformUtility.getSortMark(ManagementRequestApproverNameComparator.class.getName(), params) %></th>
			</tr>
		</thead>
		<tbody>
<%
