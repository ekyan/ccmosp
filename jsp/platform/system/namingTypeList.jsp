...
<div class="FixList" id="divList">
		...
		<tr>
			...
			<th class="ListSelectTh" id="thNamingType"><%= params.getName("Name") + params.getName("Type") %></th>
		</tr>
<%
		...
		<tr>
			<td class="ListSelectTd" id="tdButton">
				<button type="button" class="Name2Button"
					onclick="submitTransfer(event, null, null, new Array('<%= PlatformConst.PRM_TRANSFERRED_TYPE %>', '<%=HtmlUtility.escapeHTML(vo.getAryLblNamingTypeName()[i][0]) %>'), '<%= NamingTypeListAction.CMD_TRANSFER %>')"><%= params.getName("Select") %></button>
			</td>
