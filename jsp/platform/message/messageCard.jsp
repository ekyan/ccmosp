...
<div class="List" id="divEdit">
	<table class="ListTable">
		...
		<tr>
			<td class="TitleTd"><span class="RequiredLabel">*&nbsp;</span><%= params.getName("Open") %><%= params.getName("Start") %><%= params.getName("Day") %></td>
			...
			<td class="TitleTd"><span class="RequiredLabel">*&nbsp;</span><%= params.getName("Open") %><%= params.getName("End") %><%= params.getName("Day") %></td>
			<td class="InputTd">
			...
			<td class="TitleTd"><label for="pltMessageType"><%= params.getName("Message") %><%= params.getName("Type") %></label></td>
			<td class="InputTd">
		...
		<tr>
			<td class="TitleTd"><%= params.getName("Message") %><%= params.getName("No") %></td>
		...
		<tr>
			<td class="TitleTd"><label for="pltInactivate"><%= params.getName("Effectiveness") %><%= params.getName("Slash") %><%= params.getName("Inactivate") %></label></td>
			<td class="InputTd">
	...
	<table class="ListTable">
		<tr>
			<th class="ListTableTh" colspan="5">
				<span class="TitleTh"><%= params.getName("Apply") %><%= params.getName("Range") %></span>
		...
		<tr>
			...
			<td class="TitleTd"><label for="txtEmployeeCode"><%= params.getName("Personal") + params.getName("FrontParentheses") + params.getName("InputCsv") + params.getName("BackParentheses") %></label></td>
		...
		<tr>
			<td class="TitleTd"><%= params.getName("Display") %><%= params.getName("TargetPerson") %><%= params.getName("FullName") %></td>
			<td class="TitleInputTd" colspan="3" id="lblEmployeeName"><%= HtmlUtility.escapeHTML(vo.getLblEmployeeName()) %></td>
		</tr>
	</table>
	<table class="ListTable">
		<tr>
			<th class="ListTableTh"><span class="TitleTh"><%= params.getName("Message") %><%= params.getName("Set") %></span><a></a></th>
		</tr>
		<tr>
			<td class="InputTd" id="tdOver"><input type="text" class="Name30RequiredTextBox" id="txtMessageTitle" name="txtMessageTitle" value="<%= HtmlUtility.escapeHTML(vo.getTxtMessageTitle()) %>" onFocus="hideFormGuide(this, '<%= params.getName("Title") + params.getName("InputSomething") %>');" onBlur="showFormGuide(this, '<%= params.getName("Title") + params.getName("InputSomething") %>');"/></td>
		</tr>
		<tr>
			<td class="InputTd" id="tdUnder"><textarea maxlength="200" onkeyup="return isMaxlength(this)" rows="3" cols="50" class="Name200TextArea" id="txtMessage" name="txtMessage"  onFocus="hideFormGuide(this, '<%= params.getName("Message") + params.getName("InputSomething") %>');" onBlur="showFormGuide(this, '<%= params.getName("Message") + params.getName("InputSomething") %>');"><%= HtmlUtility.escapeHTML(vo.getTxtMessage()) %></textarea></td>
		</tr>
	</table>
</div>
<div class="Button">
	...
	<button type="button" class="Name7Button" id="btnMessageList" name="btnMessageList" onclick="submitTransfer(event, null, null, null, '<%= MessageListAction.CMD_RE_SEARCH %>')"><%= params.getName("Message") %><%= params.getName("List") %></button>
</div>
