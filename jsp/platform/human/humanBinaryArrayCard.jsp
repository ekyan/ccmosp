	...
	<table class="ListTable" id="tblActivateDate">
		<tr>
			...
			<td class="InputTd" id="inputActivateDate">
				...
				<button type="button" class="Name2Button" id="btnActivateDate" onclick="submitForm(event, 'inputActivateDate', null, '<%= HumanBinaryArrayCardAction.CMD_SET_ACTIVATION_DATE %>')"><%= vo.getModeActivateDate().equals(PlatformConst.MODE_ACTIVATE_DATE_FIXED) ? params.getName("Change") : params.getName("Decision") %></button>
			</td>
		</tr>
	</table>
</div>
...
<div class="List">
	<table class="ListTable" id="tblActivateDate">
		<tr>
			...
			<td>	
				<button type="button" class="Name4Button" id="btnOutput" onclick="submitFile(event, null, new Array('<%=PlatformConst.PRM_TRANSFERRED_INDEX%>','<%= vo.getRowId() %>'), '<%= HumanBinaryOutputFileAction.CMD_ARRAY_FILE %>');"><%= params.getName("Output") %></button>
			</td>
		</tr>
	</table>
</div>
...
%>
	<button type="button" id="btnRegist" class="Name4Button" onclick="submitFormMulti(event, 'tblCard', checkExtra, '<%= HumanBinaryArrayCardAction.CMD_ADD %>')"><%= params.getName("Insert") %></button>
<%
} else if (vo.getModeCardEdit().equals(PlatformConst.MODE_CARD_EDIT_EDIT)) {
%>
	<button type="button" id="btnRegist" class="Name4Button" onclick="submitRegist(event, 'tblCard', null, '<%= HumanBinaryArrayCardAction.CMD_UPDATE %>')"><%= params.getName("Update") %></button>
	<button type="button" id="btnDelete" class="Name4Button" onclick="submitDelete(event, null, null, '<%=HumanBinaryArrayCardAction.CMD_DELETE%>')"><%=params.getName("Delete")%><%}%></button>
