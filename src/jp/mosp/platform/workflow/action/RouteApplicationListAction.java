...
public class RouteApplicationListAction extends PlatformSystemAction {
	 ...
	 * 特殊表示コマンド。<br>
	 * <br>
	 * メニューバーより「承認経路一覧」の項目を選択した場合はこのコマンドが実行され、承認経路一覧画面として初期表示を行う。		<br>
	 * 検索欄のテーブルは承認経路一覧用のものを表示し、ルート名称のリンク文字クリック時はルート適用詳細画面の特殊表示コマンドが実行されるようにする。<br>
	 */
	// public static final String	CMD_SPECIAL_DISPLAY	= "PF3316";
	 ...
	 * 検索欄のテーブルは承認経路一覧用のものを表示し、ルート名称のリンク文字クリック時はルート適用詳細画面の特殊表示コマンドが実行されるようにする。<br>
	 */
	// public static final String	CMD_SPECIAL_DISP	= "PF3411";
