...
public class TemplateOutputBean extends PlatformBean implements TemplateOutputBeanInterface {
	 ...
	 */
	public TemplateOutputBean(MospParams mospParams, Connection connection) {
		super(mospParams, connection);
	}
	
	@Override
	public void initBean() throws MospException {
		importReference = (ImportReferenceBeanInterface)createBean(ImportReferenceBeanInterface.class);
		importFieldReference = (ImportFieldReferenceBeanInterface)createBean(ImportFieldReferenceBeanInterface.class);
	}
	...
	protected String[] getHeader(String importTable, List<ImportFieldDtoInterface> list) throws MospException {
		...
		for (int i = 0; i < array.length; i++) {
			...
			if (fieldNames.length > 1) {
				array[i] = mospParams.getName(fieldNames[1], "FrontParentheses", fieldNames[0], "BackParentheses");
				continue;
			}
			// コードから取得
