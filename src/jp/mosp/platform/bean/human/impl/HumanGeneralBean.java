...
public class HumanGeneralBean extends PlatformHumanBean implements HumanGeneralBeanInterface {
	 ...
	 */
	protected HumanGeneralBean(MospParams mospParams, Connection connection) {
	...
	public void initBean() throws MospException {
		...
		historyDao = (HumanHistoryDaoInterface)createDao(HumanHistoryDaoInterface.class);
		arrayDao = (HumanArrayDaoInterface)createDao(HumanArrayDaoInterface.class);
	}
	...
	public Map<String, String[][]> getInputActiveDateGeneralPulldown(String division, String viewKey) {
		...
		for (TableItemProperty tableItem : tableItemList) {
			...
			for (int i = 0; i < itemKeys.length; i++) {
				...
				// 対象項目プルダウン設定
				String[][] pulldown = { { "", mospParams.getName("InputActiveDate") } };
				// マップにつめる
				pulldownMap.put(itemNames[i], pulldown);
			}
		}
	...
	public String getSeparateTxtItemArrayValue(String personalId, String itemName, ItemProperty itemProperty,
		...
		if (dataType.equals(KEY_CONCATENATED_LABEL)) {
			...
			for (int i = 0; i < itemNames.length; i++) {
				...
				// 人事汎用一覧情報取得
				HumanArrayDtoInterface itemDto = arrayDao.findForKey(personalId, itemNames[i], getInteger(rowId));
				if (itemDto != null) {
					// プルダウン値取得
					String pulldownValue = getPulldownValue(labelItemProperty, targetDate, itemDto.getHumanItemValue(),
							itemNames[i], true);
	...
	public String getFormatHyphenValue(String value) {
		// 値がありかつ空白でない場合
		if (value != null && value.equals("") == false && value.isEmpty() == false) {
			// 値 + -
			return value + mospParams.getName("Hyphen");
		} else {
			// 値が無い場合空白
			return "";
		}
	...
	public String[][] getPulldownForHumanExportImport(String division, String viewKey) {
		...
		for (int i = 0; i < nameSize; i++) {
			array[i][0] = division + MospConst.APP_PROPERTY_SEPARATOR + tableItemKeys[i];
			array[i][1] = mospParams.getName(tableItemKeys[i], "FrontParentheses", division, "BackParentheses");
		}
		return array;
	}
