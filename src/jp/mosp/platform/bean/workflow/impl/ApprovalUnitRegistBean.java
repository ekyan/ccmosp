...
public class ApprovalUnitRegistBean extends PlatformHumanBean implements ApprovalUnitRegistBeanInterface {
	...
	public void validate(ApprovalUnitDtoInterface dto, Integer row) throws MospException {
		// 必須確認(ユニットコード)
		checkRequired(dto.getUnitCode(), getNameUnitCode(), row);
		// 必須確認(有効日)
		checkRequired(dto.getActivateDate(), getNameActivateDate(), row);
		// 必須確認(ユニット区分)
		checkRequired(dto.getUnitType(), getNameUnitCode(), row);
		// 桁数確認(ユニットコード)
		checkLength(dto.getUnitCode(), LEN_CODE, getNameUnitCode(), row);
		// 桁数確認(ユニット名称)
		checkLength(dto.getUnitName(), LEN_UNIT_NAME, getNameUnitName(), row);
		// 桁数確認(承認者所属コード)
		...
		if (dto.getUnitType().equals(PlatformConst.UNIT_TYPE_SECTION)) {
			// 必須確認(承認者所属コード)
			checkRequired(dto.getApproverSectionCode(), getNameApproverSectionCode(), row);
			// 必須確認(承認者職位コード)
			checkRequired(dto.getApproverPositionCode(), getNameApproverPositionCode(), row);
			// 所属及び職位の存在確認
			checkMaster(dto, row);
		}
		// ユニット区分確認(個人の場合)
		if (dto.getUnitType().equals(PlatformConst.UNIT_TYPE_PERSON)) {
			// 必須確認(承認者個人ID)
			checkRequired(dto.getApproverPersonalId(), getNameApproverEmployeeCode(), row);
		}
		// 必須確認(ユニット名称)
		checkRequired(dto.getUnitName(), getNameUnitName(), row);
	}
	...
	protected String getNameApproverSectionCode() {
		return mospParams.getName("Approver", "Section", "Code");
	}
	...
	protected String getNameApproverPositionCode() {
		return mospParams.getName("Approver", "Position", "Code");
	}
