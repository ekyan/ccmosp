...
public class PostalCodeRegistBean extends PlatformBean
	...
	public void initBean() throws MospException {
		// DAO準備
		dao = (PostalCodeDaoInterface)createDao(PostalCodeDaoInterface.class);
	}
	...
	@Override
	public void update(PostalCodeDtoInterface dto) throws MospException {
		// DTO妥当性確認
		validate(dto);
		if (mospParams.hasErrorMessage()) {
			return;
		...
		// 論理削除
		logicalDelete(dao, dto.getPfmPostalCodeId());
		// レコード識別ID最大値をインクリメントしてDTOに設定
		dto.setPfmPostalCodeId(dao.nextRecordId());
		// 登録処理
		dao.insert(dto);
	...
	@Override
	public void delete(PostalCodeDtoInterface dto) throws MospException {
		// 削除情報の検証
		checkDelete(dto);
		if (mospParams.hasErrorMessage()) {
			return;
	 ...
	 * @throws MospException SQLの作成に失敗した場合、或いはSQL例外が発生した場合
	 */
	protected void checkUpdate(PostalCodeDtoInterface dto) throws MospException {
	 ...
	 * @throws MospException SQLの作成に失敗した場合、或いはSQL例外が発生した場合
	 */
	protected void checkDelete(PostalCodeDtoInterface dto) throws MospException {
