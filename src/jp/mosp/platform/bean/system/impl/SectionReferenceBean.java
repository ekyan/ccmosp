...
public class SectionReferenceBean extends PlatformBean implements SectionReferenceBeanInterface {
	...
	public String[][] getSelectArrayForMaintenance(Date targetDate) throws MospException {
		...
		array[idx++][1] = mospParams.getName(NAM_REGIST_AS_GREATEST);
		// 配列作成
	...
	public boolean useDisplayName() {
		// 所属プルダウン表示内容設定取得
		String useDisplayName = mospParams.getApplicationProperty(APP_SECTION_PULLDOWN_NAME);
		// 所属プルダウン表示内容設定確認
		if (useDisplayName != null && useDisplayName.equals(SECTION_PULLDOWN_NAME_DISPLAY)) {
			return true;
		}
		return false;
	}
