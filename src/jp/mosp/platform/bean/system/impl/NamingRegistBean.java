...
public class NamingRegistBean extends PlatformBean implements NamingRegistBeanInterface {
	 ...
	 */
	public NamingRegistBean(MospParams mospParams, Connection connection) {
	...
	public void initBean() throws MospException {
		...
		humanHistoryDao = (HumanHistoryDaoInterface)createDao(HumanHistoryDaoInterface.class);
	}
	...
	public void validate(NamingDtoInterface dto, Integer row) {
		// 必須確認(名称区分)
		checkRequired(dto.getNamingType(), getNamingType(), row);
		// 名称区分存在確認
		if (isExistNamingType(dto.getNamingType()) == false) {
			mospParams.addErrorMessage(PlatformMessageConst.MSG_NO_ITEM, getNamingType());
			return;
		}
		// 必須確認(名称項目コード)
		checkRequired(dto.getNamingItemCode(), getNamingItemCode(), row);
		// 必須確認(有効日)
		checkRequired(dto.getActivateDate(), getNameActivateDate(), row);
		// 必須確認(名称項目名)
		checkRequired(dto.getNamingItemName(), getNamingItemName(), row);
		// 必須確認(名称項目略称)
		checkRequired(dto.getNamingItemAbbr(), getNamingItemAbbr(), row);
		// 桁数確認(名称項目コード)
		checkLength(dto.getNamingItemCode(), LEN_NAMING_ITEM_CODE, getNamingItemCode(), row);
		// 桁数確認(名称項目名)
		checkLength(dto.getNamingItemName(), LEN_NAMING_ITEM_NAME, getNamingItemName(), row);
		// バイト数(表示上)確認(名称項目略称)
		checkByteLength(dto.getNamingItemAbbr(), LEN_NAMING_ITEM_ABBR, getNamingItemAbbr(), row);
		// 型確認(名称項目コード)
		checkTypeCode(dto.getNamingItemCode(), getNamingItemCode(), row);
		// 型確認(無効フラグ)
	 ...
	 * @return 名称区分
	 */
	protected String getNamingType() {
		return mospParams.getName("Name") + mospParams.getName("Type");
	}
	 ...
	 * @return 名称項目コード
	 */
	protected String getNamingItemCode() {
		return mospParams.getName("Name") + mospParams.getName("Item") + mospParams.getName("Code");
	}
	 ...
	 * @return 所属名称名称
	 */
	protected String getNamingItemName() {
		return mospParams.getName("Name") + mospParams.getName("Item") + mospParams.getName("Name");
	}
	 ...
	 * @return 名称項目略称
	 */
	protected String getNamingItemAbbr() {
		return mospParams.getName("Name") + mospParams.getName("Item") + mospParams.getName("Abbreviation");
	}
	 ...
	 * @return 閉鎖フラグ名称
	 */
	protected String getNameCloseFlag() {
		StringBuffer sb = new StringBuffer();
		sb.append(mospParams.getName("Effectiveness"));
		sb.append(mospParams.getName("Slash"));
		sb.append(mospParams.getName("Inactivate"));
		return sb.toString();
	}
}
