...
public class ApprovalHistoryAction extends TimeAction {
	...
	protected void approvalHistoryLieuSelectShow() throws MospException {
		...
		sb.append(DateUtility.getStringDateAndDay(dto.getRequestDate())).append(blank)
			.append(getSubHolidayTypeName(dto.getHolidayRange()));
		vo.setLblSubHolidayDate(sb.toString());
		sb = new StringBuffer();
		sb.append(setWorkDateSubHolidayType(dto.getWorkDateSubHolidayType())).append(blank);
		sb.append(DateUtility.getStringDateAndDay(dto.getWorkDate())).append(blank);
		sb.append(getSubHolidayTypeName(dto.getHolidayRange()));
	...
	protected void differenceWorkApprovalHistorySelectShow() throws MospException {
		...
		vo.setLblDifferenceTime(DateUtility.getStringTime(dto.getRequestStart(), dto.getRequestDate())
				+ mospParams.getName("Wave") + DateUtility.getStringTime(dto.getRequestEnd(), dto.getRequestDate()));
	...
	/**
	 * 申請情報が存在しない場合のエラーメッセージを設定する。<br>
	 */
	protected void addRequestNotExistErrorMessage() {
		String rep = mospParams.getName("Application", "Information");
