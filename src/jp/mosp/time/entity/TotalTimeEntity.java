...
public class TotalTimeEntity implements TotalTimeEntityInterface {
	...
	protected void addHolidayShortUnpaid() {
		...
		for (Date targetDate : targetDateList) {
			...
			// 勤務形態エンティティを取得
			WorkTypeEntity workTypeEntity = TotalTimeUtility.getLatestWorkTypeEntity(workTypeEntityMap, workTypeCode,
					targetDate);
	...
	protected void setAttendanceTransactionMap() {
		// 勤怠トランザクション登録判定情報
		attendanceTransactionMap = new HashMap<Date, String>();
		// 計算対象日毎に処理
		for (Date targetDate : targetDateList) {
			// 休職中である場合
			if (HumanUtility.isSuspension(suspensionList, targetDate)) {
	...
	protected int getOverForty(int minutes, int weeklyTotalWorkTime) {
		// 40時間を分で取得
		int forty = PREDETERMINED_OVERTIME_WORK * TimeConst.CODE_DEFINITION_HOUR;
	...
	protected void setInitFileds() {
		...
		requestEntityMap = new HashMap<Date, RequestEntity>();
	}
	...
	protected RequestEntity getRequestEntity(Date targetDate) {
		// 申請エンティティを取得
		RequestEntity entity = requestEntityMap.get(targetDate);
		// 申請エンティティが取得できた場合
		if (entity != null) {
			// 申請エンティティを取得
			return entity;
