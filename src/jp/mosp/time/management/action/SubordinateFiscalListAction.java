...
public class SubordinateFiscalListAction extends TimeSettingAction {
	...
	protected void setVoList(List<? extends BaseDtoInterface> list) throws MospException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			aryLblPaidHolidayDays[i] = getFormatDaysAndHours(dto.getPaidHolidayDays(), dto.getPaidHolidayTime(), true);
			aryLblPaidHolidayRestDays[i] = getFormatDaysAndHours(dto.getPaidHolidayRestDays(),
					dto.getPaidHolidayRestTime(), true);
			aryLblStockHolidayDays[i] = getFormatDaysAndHours(dto.getStockHolidayDays(), 0, true);
			aryLblStockHolidayRestDays[i] = getFormatDaysAndHours(dto.getStockHolidayRestDays(), 0, true);
			aryLblSeasonHolidayDays[i] = getFormatDaysAndHours(dto.getSeasonHolidayDays(), 0, true);
			aryLblSeasonHolidayRestDays[i] = getFormatDaysAndHours(dto.getSeasonHolidayRestDays(),
					dto.getSeasonHolidayRestHours(), true);
		}
		// データをVOに設定
		// 一覧表示項目設定
