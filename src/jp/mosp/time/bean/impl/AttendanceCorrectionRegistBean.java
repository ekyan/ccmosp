...
public class AttendanceCorrectionRegistBean extends PlatformBean implements AttendanceCorrectionRegistBeanInterface {
	...
	protected AttendanceCorrectionDtoInterface setChangePointGoOut(AttendanceCorrectionDtoInterface dto,
		...
		// 分単位休暇A
		if (TimeConst.CODE_GO_OUT_MINUTELY_HOLIDAY_A == newGoOutDto.getGoOutType()) {
			if (newGoOutDto.getTimesGoOut() == 1) {
				dto.setCorrectionType(TimeConst.CODE_ATTENDANCE_ITEM_NAME_MINUTELY_HOLIDAY_A1);
			}
			if (newGoOutDto.getTimesGoOut() == 2) {
				dto.setCorrectionType(TimeConst.CODE_ATTENDANCE_ITEM_NAME_MINUTELY_HOLIDAY_A2);
			}
			if (newGoOutDto.getTimesGoOut() == 3) {
				dto.setCorrectionType(TimeConst.CODE_ATTENDANCE_ITEM_NAME_MINUTELY_HOLIDAY_A3);
			}
			if (newGoOutDto.getTimesGoOut() == 4) {
				dto.setCorrectionType(TimeConst.CODE_ATTENDANCE_ITEM_NAME_MINUTELY_HOLIDAY_A4);
			}
		}
		// 分単位休暇B
		if (TimeConst.CODE_GO_OUT_MINUTELY_HOLIDAY_B == newGoOutDto.getGoOutType()) {
			if (newGoOutDto.getTimesGoOut() == 1) {
				dto.setCorrectionType(TimeConst.CODE_ATTENDANCE_ITEM_NAME_MINUTELY_HOLIDAY_B1);
			}
			if (newGoOutDto.getTimesGoOut() == 2) {
				dto.setCorrectionType(TimeConst.CODE_ATTENDANCE_ITEM_NAME_MINUTELY_HOLIDAY_B2);
			}
			if (newGoOutDto.getTimesGoOut() == 3) {
				dto.setCorrectionType(TimeConst.CODE_ATTENDANCE_ITEM_NAME_MINUTELY_HOLIDAY_B3);
			}
			if (newGoOutDto.getTimesGoOut() == 4) {
				dto.setCorrectionType(TimeConst.CODE_ATTENDANCE_ITEM_NAME_MINUTELY_HOLIDAY_B4);
			}
		}
		if (oldGoOutDto != null) {
