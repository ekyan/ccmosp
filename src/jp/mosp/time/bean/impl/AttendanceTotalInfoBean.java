...
public class AttendanceTotalInfoBean extends TimeBean implements AttendanceTotalInfoBeanInterface {
	...
	public List<AttendanceTotalVo> setFiscalYearAttendanceTotalList(String personalId, int fiscalYear)
		...
		for (int i = 0; i < 12; i++) {
			...
			paidHolidayTime += getFloatValue(attendanceListDto.getPaidHolidayTime());
	...
	/**
	 * 回数文字列を取得する(Double→String)。<br>
	 * 回数を文字列(小数点以下1桁)で表す。<br>
	 * @param times 対象回数
	 * @return 回数文字列
	 */
	protected String getStringDoubleTimes(Double times) {
		// 回数確認
		if (times == null) {
			// 回数がnullならハイフン
			return getHyphenNaming();
	 ...
	 * 値がnullの場合0を返す。<br>
	 * @param value 値
	 * @return float型に変換した値（nullの場合0.0F)
	 */
	protected float getFloatValue(Float value) {
		if (value == null) {
			return 0.0F;
		}
