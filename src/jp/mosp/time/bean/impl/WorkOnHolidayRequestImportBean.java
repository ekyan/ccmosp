	...
	protected WorkOnHolidayRequestDtoInterface getWorkOnHolidayRequestSubstituteOffDto(
		...
		if (startDate == null) {
			mospParams.addErrorMessage(PlatformMessageConst.MSG_INPUT_FORM_AMP,
					mospParams.getName("GoingWork", "Schedule", "Moment"));
			return dto;
		...
		// 起算時刻取得
		CutoffUtilBeanInterface cutoffUtil = (CutoffUtilBeanInterface)createBean(CutoffUtilBeanInterface.class);
		TimeSettingDtoInterface timeSettingDto = cutoffUtil.getTimeSetting(personalId, requestDate);
		Date startDayTime = timeSettingDto.getStartDayTime();
		...
		if (endDate == null) {
			mospParams.addErrorMessage(PlatformMessageConst.MSG_INPUT_FORM_AMP,
					mospParams.getName("RetireOffice", "Schedule", "Moment"));
			return dto;
			// 退勤予定時刻が出勤予定時刻より前の場合、起算時刻をこえている場合エラーメッセージを追加
