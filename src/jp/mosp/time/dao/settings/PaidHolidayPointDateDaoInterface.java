...
public interface PaidHolidayPointDateDaoInterface extends BaseDaoInterface {
	 ...
	 * 有給休暇基準日管理マスタ取得。
	 * <p>
	 * 有給休暇コードと有効日と基準日経過回数から有給休暇基準日管理マスタを取得する。
	 * </p>
	 * @param paidHolidayCode 有給休暇コード
	 * @param activateDate 有効日
	 * @param timesPointDate 基準日経過回数
	 ...
	 * @throws MospException SQLの作成に失敗した場合、或いはSQL例外が発生した場合
	 */
	PaidHolidayPointDateDtoInterface findForInfo(String paidHolidayCode, Date activateDate, int timesPointDate)
			throws MospException;
	 ...
	 * 条件による検索。
	 * <p>
	 * 検索条件から有給休暇基準日管理マスタリストを取得する。
	 * </p>
	 * @param param 検索条件
	 * @return 有給休暇基準日管理マスタリスト
	 */
	List<PaidHolidayPointDateDtoInterface> findForSearch(Map<String, Object> param);
	
	/**
	 * 検索条件取得。
	 * @return 有給休暇基準日管理検索条件マップ
	 */
