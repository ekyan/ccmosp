...
public class TmdDifferenceRequestDao extends PlatformDao implements DifferenceRequestDaoInterface {
	...
	@Override
	public List<DifferenceRequestDtoInterface> findForWorkflowStatus(String personalId, int workflowStage,
		...
		try {
			...
			setParam(index++, TimeConst.CODE_FUNCTION_DIFFERENCE);
			executeQuery();
			return mappingAll();
		} catch (Throwable e) {
			throw new MospException(e);
	...
	@Override
	public List<DifferenceRequestDtoInterface> findForTerm(String personalId, Date firstDate, Date lastDate)
			throws MospException {
		try {
			index = 1;
			StringBuffer sb = getSelectQuery(getClass());
