...
public class TmmApplicationDao extends PlatformDao implements ApplicationDaoInterface {
	...
	@Override
	public ApplicationDtoInterface findForInfo(String applicationCode, Date activateDate) throws MospException {
		try {
			index = 1;
			StringBuffer sb = getSelectQuery(getClass());
			sb.append(where());
	...
	@Override
	public List<ApplicationDtoInterface> findForInfoList(String workSettingCode, Date targetDate) throws MospException {
		try {
			index = 1;
			StringBuffer sb = getSelectQuery(getClass());
			sb.append(where());
