 ...
 * 時差出勤申請及び連続取得設定を確認し、警告メッセージを表示する。<br>
...
function submitApplication(event, cmd) {
	...
	var continueMessage = "";
	// 連続取得が必須或いは警告の場合
	if (jsHolidayContinue == "0" || jsHolidayContinue == "1") {
		var requestDay = parseIntDecimal(jsHolidayTerm);
		if (requestDay < parseFloat(jsHolidayRemainDay)) {
			if (jsHolidayContinue == "0") {
				// 必須
				continueMessage = getMessage(MSG_HOLIDAY_REQUIRED_CHECK, "");
			} else if (jsHolidayContinue == "1") {
				// 警告
