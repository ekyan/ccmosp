 ...
 * @return 無し
 */
function checkPaidHolidayGiving(aryMessage, event) {
	var errFlg = 0
	// 前年度付与日数チェック
	if(checkGiving(aryMessage, event, "txtEditFormerGivingDay") == false) {
		errFlg = 1;
	}
	// 今年度付与日数チェック
	if(checkGiving(aryMessage, event, "txtEditGivingDay") == false) {
		errFlg = 1;
	}
	// ストック付与日数チェック
	if(checkGiving(aryMessage, event, "txtEditGivingStockDay") == false) {
		errFlg = 1;
	}
	if(errFlg == 1) {
		return;
 ...
 * @param event イベントオブジェクト
 */
function onChangeGiving(event) {
	var aryMessage= new Array();
//	checkNumeric("txtEditFormerGivingDay", aryMessage);
	checkGiving(aryMessage, event, "txtEditFormerGivingDay");
	checkNumber("txtEditFormerGivingTime", aryMessage);
//	checkNumeric("txtEditGivingDay", aryMessage);
	checkGiving(aryMessage, event, "txtEditGivingDay");
	checkNumber("txtEditGivingTime", aryMessage);
//	checkNumeric("txtEditGivingStockDay", aryMessage);
	checkGiving(aryMessage, event, "txtEditGivingStockDay");
